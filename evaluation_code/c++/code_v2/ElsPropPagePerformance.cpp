...
IMPLEMENT_DYNCREATE(CElsPropPagePerformance, CNSFlexPropertyPage)

BEGIN_MESSAGE_MAP(CElsPropPagePerformance, CNSFlexPropertyPage)
	//{{AFX_MSG_MAP(CElsPropPagePerformance)
...
CElsPropPagePerformance::CElsPropPagePerformance() :
	CNSFlexPropertyPage(CElsPropPagePerformance::IDD)
{
	...
	//{{AFX_DATA_INIT(CElsPropPagePerformance)
...
void CElsPropPagePerformance::DoDataExchange(CDataExchange* pDX)
{
	CPropertyPage::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CElsPropPagePerformance)
...
BOOL CElsPropPagePerformance::OnInitDialog() 
{
...
void CElsPropPagePerformance::OnUpdate(CView* pSender, LPARAM lHint, CObject* pHint) 
{
	...
	{
		CWaitCursor wait;
		if (pDoc->m_PIArray[i].PIPreference < 0.0)
		{
		...
		for (j = 0; j < pDoc->m_nOfBlocks; j++)
		{
			if (m_CHECK_ElsPreferenceSubstracted)
			{
				if (m_CHECK_ElsTransformArtanh)
					x = TransformArtanh(pDoc->m_PIArray[i].PI[j] - 
						pDoc->m_PIArray[i].PIPreference);
				else
					x = pDoc->m_PIArray[i].PI[j] - pDoc->m_PIArray[i].PIPreference;
			}
			else
			{
				if (m_CHECK_ElsTransformArtanh)
			...
			sx_2[j] = sx[j] * sx[j];
			if (nIncl < 2) continue;
			pDoc->m_PI_SEM[j] = s_x2[j] - (sx_2[j] / (double)nIncl);
			pDoc->m_PI_SEM[j] /= (double)(nIncl * (nIncl - 1));
			pDoc->m_PI_SEM[j] = sqrt(pDoc->m_PI_SEM[j]);
		}
	}
	...
	for (j = 0; j < pDoc->m_nOfBlocks; j++)
	{
		...
		s.Format("PI%0i: % .3f +/- %.3f (%s)", j + 1, pDoc->m_PI_Mean[j],
			pDoc->m_PI_SEM[j], t);
...
void CElsPropPagePerformance::DrawPerformanceHistogram()
{
	CElsDoc* pDoc = (CElsDoc*)GetDocument();
	if (pDoc == NULL) return;
	ASSERT(pDoc);
...
void CElsPropPagePerformance::UpdateDocumentData()
{
	CElsDoc* pDoc = (CElsDoc*)GetDocument();
	ASSERT(pDoc);
...
void CElsPropPagePerformance::OnCheckElsHeat0180() 
{
...
void CElsPropPagePerformance::OnCheckElsHeat90270() 
{
	// TODO: Add your control notification handler code here
	UpdateData();
	OnUpdate(NULL, 0, NULL);
...
void CElsPropPagePerformance::OnCheckElsNegpref() 
{
	// TODO: Add your control notification handler code here
	UpdateData();
	OnUpdate(NULL, 0, NULL);
...
void CElsPropPagePerformance::OnCheckElsPospref() 
{
	// TODO: Add your control notification handler code here
	UpdateData();
	OnUpdate(NULL, 0, NULL);
...
void CElsPropPagePerformance::OnCheckElsSubpref() 
{
	// TODO: Add your control notification handler code here
	UpdateData();
	OnUpdate(NULL, 0, NULL);
...
void CElsPropPagePerformance::OnCheckElsTransformartanh() 
{
	// TODO: Add your control notification handler code here
	UpdateData();
	OnUpdate(NULL, 0, NULL);
...
void CElsPropPagePerformance::OnPaint() 
{
...
void CElsPropPagePerformance::OnButtonElsExportPI() 
{
	// TODO: Add your control notification handler code here
	CElsExportPI dlg;
